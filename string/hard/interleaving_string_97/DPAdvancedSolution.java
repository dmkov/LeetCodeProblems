...
/**
 ...
 2. Approach
    ...
    2.4 The difference from 2D matrix is that we re-use last line for every next iteration
 3 Implementation
    ...
    public boolean isInterleave(String s1, String s2, String s3) {
        ...
        for (int i = 0; i <= s1.length(); i++) {
            for (int j = 0; j <= s2.length(); j++) {
                ...
                dp[j] = (i > 0 && s3Char == s1.charAt(i - 1) && dp[j])
                        || (j > 0 && s3Char == s2.charAt(j - 1) && dp[j-1]);
